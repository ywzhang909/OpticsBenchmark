"""
GroqLLM - Groq 托管模型调用类

支持通过 GroqProvider 进行 Groq API 调用。
"""
    ...
    async def chat(
        ...
        **kwargs: Any,
        ...
        if isinstance(provider, GroqProvider):
            return await self._chat_groq(messages, provider, **kwargs)
        raise ValueError(
            f"GroqLLM 不支持 provider: {type(provider).__name__}，"
            f"仅支持 GroqProvider"
        )
