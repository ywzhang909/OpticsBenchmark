...
class ZOSAPIEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        zos_config: ZOSConnectionConfig | None = None,
    ):
    ...
    async def setup(self) -> None:
        ...
        try:
            ...
            raise RuntimeError(f"Failed to setup ZOS-API environment: {e}") from e
    ...
    async def _check_zemax_connection(self) -> bool:
        ...
        try:
            result = sock.connect_ex(("localhost", self.zos_config.port))
            sock.close()
            return result == 0
        except Exception:
            return False
